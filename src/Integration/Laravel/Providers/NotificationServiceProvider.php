...
use Tetthys\Notification\Core\Contracts\{IdGenerator,RbacPolicy,Preferences,TemplateEngine,QueueBus,Channel};
use Tetthys\Notification\Integration\Laravel\LaravelNotifier;
use Tetthys\Notification\Integration\Laravel\Infra\{
    UuidIds, LaravelRbacConfig, DbPreferences, BladeTemplateEngine, LaravelQueueBus
};
use Tetthys\Notification\Integration\Laravel\Channels\{EmailChannel, InAppChannel};
...
final class NotificationServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->app->bind(TemplateEngine::class, fn()=> new BladeTemplateEngine());
        $this->app->bind(QueueBus::class, fn()=> new LaravelQueueBus());

        $this->app->bind('notif.channels', fn()=> [
            new EmailChannel(),
            new InAppChannel(),
        ]);
        ...
        $this->app->singleton(LaravelNotifier::class, fn($app) => new LaravelNotifier(
    ...
    {
        $this->publishes([
            __DIR__ . '/../../../../resources/config/notifications.php' => config_path('notifications.php'),
